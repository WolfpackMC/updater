...
```bash
cargo build --release          # builds both mcupdater and deploy
cargo run --bin mcupdater      # run the updater (needs INST_* env vars set, as PrismLauncher would)
...
- Remote state is a flat set of objects in the `wolfpackmc` S3 bucket, mirrored at CDN `https://wolfpack-cdn.kalkafox.dev`: `version` (u32 counter), `WFP.hash` (MD5 of the mods zip), `WFP.zip` (the mods archive), `neoforge-version` (string). `mcupdater` reads through the S3 URL directly (not the CDN) for `version`/`neoforge-version`/`WFP.zip`.
- Version comparison in `mcupdater` treats local version `0` as "unset" (always update), not "up to date" — see the `local_version >= remote_version && local_version != 0` check in `main.rs`.
- Mod sync in `mcupdater` is not a naive overwrite: existing mods identical to the incoming version are left in place; anything else present locally gets moved to a timestamped backup dir before the new set is moved in from the staging dir (`.mods_staging` under `INST_MC_DIR`).
